...
from src.config.sandbox_mode import detect_sandbox_mode


def _detect_mode_with_config(config_content: str) -> str:
    """Write config content into a temp project root and detect sandbox mode."""
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp_root = Path(tmpdir)
        config_file = tmp_root / "config.yaml"
        config_file.write_text(config_content, encoding="utf-8")
        return detect_sandbox_mode(config_file)


def test_detect_mode_defaults_to_local_when_config_missing():
    """No config file should default to local mode."""
    with tempfile.TemporaryDirectory() as tmpdir:
        assert detect_sandbox_mode(Path(tmpdir) / "config.yaml") == "local"

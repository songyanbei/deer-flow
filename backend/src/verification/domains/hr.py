...
class HrTaskVerifier(BaseVerifier):
    """Verifies hr-agent task results before they enter verified_facts."""

    name = "hr_task_verifier"
    ...
    def verify(self, ctx: VerificationContext) -> VerificationResult:
        ...
        if not result_text:
            findings.append(VerificationFinding(
                field="task_result",
                severity="error",
                message="HR task returned empty result.",
            ))
        ...
        if result_text and len(result_text) < 10:
            findings.append(VerificationFinding(
                field="task_result",
                severity="warning",
                message=f"HR task result is suspiciously short ({len(result_text)} chars).",
                actual=result_text,
            ))
        ...
        if has_error_signal and len(result_text) < 50:
            findings.append(VerificationFinding(
                field="task_result",
                severity="error",
                message="HR task result appears to be an error message rather than a real result.",
        ...
        if errors:
            return self._fail(
                ...
                f"HR task verification failed with {len(errors)} error(s).",
                findings,
            )
        return self._pass("HR task result verified.", findings=findings)

...
- Domain agents (`config.configurable.is_domain_agent=True`) do not inherit the globally cached MCP toolset; they use only agent-scoped MCP tools loaded from `src.execution.mcp_pool`
- Domain-agent execution mode is now resolved through a lightweight engine registry
- If `engine_type` is omitted, the agent stays on the current default domain-agent mode (the same path used by `meeting-agent`)
- Supported explicit `engine_type` values are `ReadOnly_Explorer`, `ReAct`, and `SOP`
- `ReadOnly_Explorer` filters write-like MCP tools (`write/create/update/cancel/delete/insert/modify`) and receives stricter read-only prompt rules
- `ReAct` and `SOP` currently reuse the same core LangChain agent factory but receive explicit mode-specific prompt constraints through the engine registry
...
`get_available_tools(groups, include_mcp, model_name, subagent_enabled, is_domain_agent)` assembles:
1. **Config-defined tools** - Resolved from `config.yaml` via `resolve_variable()`
2. **MCP tools** - From enabled MCP servers (lazy initialized, cached with mtime invalidation)
3. **Built-in tools**:
   - `present_files` - Make output files visible to user (only `/mnt/user-data/outputs`)
   - `ask_clarification` - Request clarification (top-level agent only; intercepted by ClarificationMiddleware → interrupts)
   - `request_help` - Domain-agent-only escalation tool for cross-domain dependencies or user-owned decisions

...
DeerFlow 接入 moss-hub ticket 化 SSO。用户从 moss-hub 点击入口后跳到 DeerFlow `/sso/callback`，DeerFlow 后端校验 ticket、签发本地 `df_session` Cookie、落地用户资料，并进入 `/workspace/chats/new`。
...
|---|---|---:|---|---|
| `0000` | none | `200` | redirect to `/workspace/chats/new` | `sso_login` |
...
- Set cookie and return fixed redirect `/workspace/chats/new`.
...
- On success, `router.replace(redirect || "/workspace/chats/new")`.
...
- `/api/sso/callback` success sets cookie and returns `/workspace/chats/new`.
...
- Success redirects to `/workspace/chats/new`.
...
- User lands on `/workspace/chats/new`.
...
- moss-hub 正常 ticket 可登录 DeerFlow，并进入 `/workspace/chats/new`。
